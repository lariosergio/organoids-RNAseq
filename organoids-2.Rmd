---
title: "ORGANOIDS_2, Results Project CIBER-132_mRNAseq (L13-L41)"
author: "Sergio Lario"
date: "2023-06-21"
output:
  html_document:
...
```{r IMPORTAR_CUENTAS}
hash_table_rnaseq2<- read.csv(
  paste0(dataDir,"/hash_table_rnaseq_sl_ciber132_23_06_2023.tsv"),
  sep="\t", header=TRUE)
dim(hash_table_rnaseq2)
head(hash_table_rnaseq2[1:5,c(7:35)])
countDataOrg2<- hash_table_rnaseq2[,7:35]
```
**Importo los factores de la muestras:**

```{r IMPORTAR_METADATA}
metaDataOrg2<-read.csv(
  paste0(dataDir,"/metadata_organoids2.csv"),
  sep=";", header=TRUE)
colnames(metaDataOrg2)[1]<- "name"

#str(metaDataOrg2)
metaDataOrg2$type<-factor(metaDataOrg2$type)
metaDataOrg2$Metaplasia <- factor(metaDataOrg2$Metaplasia)
metaDataOrg2$ribosomicos<- factor(metaDataOrg2$ribosomicos)
metaDataOrg2$Origen_anat<- factor(metaDataOrg2$Origen_anat)
metaDataOrg2$group<- factor(metaDataOrg2$group, levels=c ("DISPEPSIA","GASTRITIS", "METAPLASIA","ADENOK"))
str(metaDataOrg2)
#la dejo ordenada según el orden alfabético de las muestras
metaDataOrg2 <- metaDataOrg2 %>% arrange(metaDataOrg2$name)

knitr::kable(metaDataOrg2[, c(7,1,2,5)], align = "c", booktabs = T)%>%
      kable_styling()%>%
...
datos<- melt(data.frame(countDataOrg2)+1)
...
#mantengo aquellos con más de 10 counts en más de la mitad de las muestras
n_keep<- ceiling(dim(countDataOrg2)[2]/2)

keep<- rowSums(countDataOrg2 > 10) >= n_keep
countDataOrg2<-countDataOrg2[keep,]
#dseq_org1<-dseq_org1[keep,]
#dim(dseq_org1)
dim(countDataOrg2)
hash_table_Filt<- hash_table_rnaseq2[keep,]
...
# getBM tarda un tiempo en conseguir la información de los genes.
# lo tengo guardado de runs anteriores, pero si no encontrara el archivo, volvería a ejecutarse.

if (file.exists(paste0(dataDir, "/genes_of_interest.RData"))){
    load(paste0(dataDir, "/genes_of_interest.RData"))
}else{
#recojo en una lista las ID de ensembl:
    ...
    genes_of_interest=getBM(attributes=c("hgnc_symbol","ensembl_gene_id","ensembl_gene_id_version",
                                         "ensembl_transcript_id_version", "transcript_length",
                                         "transcript_is_canonical"), 
                                          filters="ensembl_gene_id", 
                                          values=ensembl_list, mart=human)
    save(genes_of_interest, file = paste0(dataDir, "/genes_of_interest.RData"))
}
...
table(duplicated(genes_of_interest_filt$ensembl_gene_id))
#mantengo los no duplicados
genes_of_interest_filt<-genes_of_interest_filt[!duplicated(genes_of_interest_filt$ensembl_gene_id),]
...
a<-merge(hash_table_Filt, genes_of_interest_filt, by= "ensembl_gene_id", all= FALSE)
...
muestras<- colnames(hash_table_Filt[c(11:39)])

for (i in muestras) {
  fila<-which(metaDataOrg2$name == i)
  ribosomico<-as.numeric(metaDataOrg2[fila,5])
  grupo<-metaDataOrg2[fila,4]
...
ggarrange(mis_plots[[9]], mis_plots[[10]], mis_plots[[11]], mis_plots[[12]],
                      ncol = 1, nrow = 4)
ggarrange(mis_plots[[13]], mis_plots[[14]], mis_plots[[15]], mis_plots[[16]],
                      ncol = 1, nrow = 4)
ggarrange(mis_plots[[17]], mis_plots[[18]], mis_plots[[19]], mis_plots[[20]],
                      ncol = 1, nrow = 4)
ggarrange(mis_plots[[21]], mis_plots[[22]], mis_plots[[23]], mis_plots[[24]],
                      ncol = 1, nrow = 4)
ggarrange(mis_plots[[25]], mis_plots[[26]], mis_plots[[27]], mis_plots[[28]],
                      ncol = 1, nrow = 4)
ggarrange(mis_plots[[29]],ncol = 1, nrow = 4)
...
```{r echo=FALSE}
library(reshape2)
datos<- hash_table_Filt[, c(9,11:39)]
...
datos$ribosomicos<-metaDataOrg2[match(datos$variable,metaDataOrg2$name),5]
...
```{r echo=FALSE}
numero<-table(hash_table_Filt$T02B>10)[2]
numero<- c(numero,table(hash_table_Filt$T04A>10)[2])
numero<- c(numero,table(hash_table_Filt$T08A>10)[2])
numero<- c(numero,table(hash_table_Filt$T11A >10)[2])
numero<- c(numero,table(hash_table_Filt$T12C>10)[2])
numero<- c(numero,table(hash_table_Filt$T14B>10)[2])
numero<- c(numero,table(hash_table_Filt$T19A>10)[2])
numero<- c(numero,table(hash_table_Filt$T20T>10)[2])
numero<- c(numero,table(hash_table_Filt$T22T>10)[2])
numero<- c(numero,table(hash_table_Filt$T23T>10)[2])
numero<- c(numero,table(hash_table_Filt$T25T>10)[2])
numero<- c(numero,table(hash_table_Filt$T27T>10)[2])
numero<- c(numero,table(hash_table_Filt$T30T>10)[2])
numero<- c(numero,table(hash_table_Filt$T31T>10)[2])
numero<- c(numero,table(hash_table_Filt$T32T>10)[2])
numero<- c(numero,table(hash_table_Filt$T35T>10)[2])
numero<- c(numero,table(hash_table_Filt$T36T>10)[2])
numero<- c(numero,table(hash_table_Filt$T50T>10)[2])
numero<- c(numero,table(hash_table_Filt$T51A>10)[2])
numero<- c(numero,table(hash_table_Filt$T52T>10)[2])
numero<- c(numero,table(hash_table_Filt$T58T>10)[2])
numero<- c(numero,table(hash_table_Filt$T59A>10)[2])
numero<- c(numero,table(hash_table_Filt$T60T>10)[2])
numero<- c(numero,table(hash_table_Filt$T63T>10)[2])
numero<- c(numero,table(hash_table_Filt$T64T>10)[2])
numero<- c(numero,table(hash_table_Filt$T65T>10)[2])
numero<- c(numero,table(hash_table_Filt$T66T>10)[2])
numero<- c(numero,table(hash_table_Filt$T67T>10)[2])
numero<- c(numero,table(hash_table_Filt$T71T>10)[2])



NumGenes<- data.frame(suma= colSums(hash_table_Filt[11:39]))
NumGenes$num_10<-numero
NumGenes$muestra<- rownames(NumGenes)
NumGenes$type<- metaDataOrg2$type
NumGenes$ribo<- metaDataOrg2$ribosomicos
...
**Comprobar que los nombres de las columnas de `countDataOrg2` coinciden con los de las filas de `metaDataOrg2`:**


```{r COMPROBACION}
all.equal(colnames(countDataOrg2), rownames(metaDataOrg2))
# no coinciden, los rownames están vacíos.
rownames(metaDataOrg2)<- metaDataOrg2$name
all.equal(colnames(countDataOrg2), rownames(metaDataOrg2))
#  ahora sí
```
...
```{r OBJETO_DESEQ}
#metaDataOrg2<-metaDataOrg2[,c(2,3,5)]

dseq_org1<- DESeqDataSetFromMatrix(countData = round(hash_table_Filt[,11:39]), 
                                   colData = metaDataOrg2,
...
the_bars_type <- ifelse(as.numeric(as.character(metaDataOrg2$type)),"red", "green")
#the_bars_meta <- ifelse(as.numeric(as.character(metaDataOrg2$Metaplasia)), "yellow", "blue")
the_bars_ribo <- ifelse(tt<- as.numeric(as.character(metaDataOrg2$ribosomicos)), "yellow", "blue")
tt
